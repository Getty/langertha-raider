...

{
  ...
  async sub run_f {
    ...
    if ($self->type eq 'final') {
      ...
      return Langertha::Raider::Result->final($text);
    }
    if ($self->type eq 'question') {
      return Langertha::Raider::Result->question($self->content // 'question?');
    }
    if ($self->type eq 'pause') {
      return Langertha::Raider::Result->pause($self->content // 'pause');
    }
  ...
  my $seq = Langertha::Raid::Sequential->new(steps => [
    Test::Runnable::Step->new(
      ...
      result_cb => sub {
        ...
        return Langertha::Raider::Result->final('stage1');
      },
    ),
    Test::Runnable::Step->new(
      name => 'reader',
      result_cb => sub {
        my ( $ctx ) = @_;
        return Langertha::Raider::Result->final('stage2:' . ($ctx->state->{shared} // 'no'));
      },
    ),
  ]);
  ...
  my $parallel = Langertha::Raid::Parallel->new(steps => [
    Test::Runnable::Step->new(
      ...
      result_cb => sub {
        ...
        return Langertha::Raider::Result->final('L');
      },
    ),
    Test::Runnable::Step->new(
      name => 'right',
      result_cb => sub {
        ...
        return Langertha::Raider::Result->final('R');
      },
    ),
  ]);
...
subtest 'Loop max iterations and stop callback' => sub {
  my $step = Test::Runnable::Step->new(
    ...
    result_cb => sub {
      ...
      return Langertha::Raider::Result->final('n=' . $ctx->state->{n});
    },
  );
    ...
    steps     => [
      Langertha::Raid::Sequential->new(steps => [
        Test::Runnable::Step->new(
          ...
          result_cb => sub {
            my ( $ctx ) = @_;
            return Langertha::Raider::Result->final(($ctx->input // '') . '.');
          },
        ),
      ]),
    ],
